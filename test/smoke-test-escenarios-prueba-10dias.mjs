...
const html = readFileSync(path.join(ROOT, 'escenarios-prueba-10dias.html'), 'utf-8');
const dom = new JSDOM(html, { url: 'http://localhost:8098/escenarios-prueba-10dias.html', pretendToBeVisual: true });
const { window } = dom;
...
await import(path.join(ROOT, 'js', 'escenario-app-prueba-10dias.js') + `?t=${Date.now()}`);
await esperar(500);
...
console.log('\n12. Historial: atrás/adelante y navegación entre módulos');
{
  ...
  assert(document.querySelector('nav.historial-nav a[href="escenarios.html"]') !== null, 'Existe el enlace de navegación de vuelta a Escenarios vigente');
}
