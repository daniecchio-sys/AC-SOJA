// ============================================================================
// test/smoke-test-prueba-10dias.mjs
// Verificación de la PRUEBA METODOLÓGICA REVERSIBLE de ventanas de 10 días
// (explorar-prueba-10dias.html / app-prueba-10dias.js). No reemplaza
// smoke-test-ui.mjs (Explorar vigente, 5 días) -- confirma que la variante
// experimental funciona de punta a punta reutilizando el mismo motor.
// Requiere el proyecto servido por HTTP en localhost:8098.
// Se ejecuta con: node test/smoke-test-prueba-10dias.mjs
// ============================================================================
...
console.log('Preparando DOM (jsdom) y globals de app-prueba-10dias.js...');
const html = readFileSync(path.join(ROOT, 'explorar-prueba-10dias.html'), 'utf-8');
const dom = new JSDOM(html, { url: 'http://localhost:8098/explorar-prueba-10dias.html', pretendToBeVisual: true });
const { window } = dom;
...
await import(path.join(ROOT, 'js', 'app-prueba-10dias.js') + `?t=${Date.now()}`);
await esperar(500);
...
console.log(checksFailed === 0
  ? '\n✓ LA PRUEBA DE VENTANAS DE 10 DÍAS FUNCIONA DE PUNTA A PUNTA (REVERSIBLE, NO REEMPLAZA EXPLORAR).'
